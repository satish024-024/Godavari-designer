import { ui, login, register, loginWithPhoneOtp, triggerRender, showToast } from "../services/store.js";
...
export function renderAuth() {
  ...
  if (authMode === "phone") {
    titleText = "Sign In with Mobile";
    subtitleText = "Enter your phone number for instant 1-tap OTP verification.";
    formContentHtml = `
      <form id="phoneOtpLoginForm" style="display: grid; gap: 18px;">
        <div style="background: rgba(200, 161, 90, 0.08); border: 1px solid rgba(200, 161, 90, 0.3); border-radius: 8px; padding: 12px 14px; font-size: 13px; color: var(--navy); display: flex; align-items: center; gap: 10px;">
          <span style="color: var(--gold);">${icon("shield-check", 20)}</span>
          <span><strong>Fast & Secure:</strong> No passwords needed. Perfect for boutique owners & smartphones.</span>
        </div>

        <label class="auth-label">
          <span>Mobile Phone Number</span>
          <div class="auth-input-wrapper">
            <span class="auth-input-icon" style="font-size: 13px; font-weight: 700; color: var(--navy); left: 12px;">+91</span>
            <input type="tel" id="authPhoneNumber" name="phone" value="${attr(enteredPhone)}" required placeholder="83098 97055" maxlength="10" class="auth-input" style="padding-left: 48px; font-size: 15px; font-weight: 600;" autocomplete="tel" ${otpSent ? "disabled" : ""} />
          </div>
        </label>
            ...
            ? `
              <label class="auth-label">
                <span style="display: flex; justify-content: space-between;">
                  ...
                  <button type="button" id="resendOtpBtn" class="auth-link">Change Number / Resend</button>
                </span>
                <div class="auth-input-wrapper">
                  <span class="auth-input-icon">${icon("lock", 17)}</span>
                  <input type="text" id="authOtpCode" name="otp" required placeholder="123456" maxlength="6" class="auth-input" style="letter-spacing: 4px; font-size: 18px; font-weight: 700; text-align: center;" autocomplete="one-time-code" />
                </div>
                <span style="font-size: 11px; color: var(--ink-soft); margin-top: 2px;">Enter the SMS verification code (or test code <strong>123456</strong>)</span>
              </label>

              <button type="submit" class="auth-submit-btn" ${isSubmitting ? "disabled" : ""}>
                ${isSubmitting ? `<div class="spinner"></div> Verifying Code...` : "Verify & Access Account"}
              </button>
            `
            : `
              <button type="submit" class="auth-submit-btn" ${isSubmitting ? "disabled" : ""}>
                ${isSubmitting ? `<div class="spinner"></div> Sending OTP...` : "Send Login OTP"}
              </button>
            `
        }
        ...
        <div class="auth-footer-text">
          ...
          <button type="button" id="toEmailSignInBtn" class="auth-link" style="margin-left: 4px;">Sign in with Email & Password</button>
        </div>
      </form>
    `;
  } else if (authMode === "signin") {
    titleText = "Email Sign In";
    ...
    formContentHtml = `
      <form id="customerLoginForm" style="display: grid; gap: 20px;">
        ...
        <div class="auth-footer-text" style="display: flex; flex-direction: column; gap: 8px;">
          <button type="button" id="toPhoneOtpBtn" class="auth-link" style="color: var(--navy); font-weight: 700;">📱 Sign in with Mobile Phone OTP (Faster)</button>
          <div>
...
export function initAuthDelegates() {
  ...
  const toPhoneOtpBtn = document.getElementById("toPhoneOtpBtn");
  if (toPhoneOtpBtn) {
    toPhoneOtpBtn.addEventListener("click", () => {
      authMode = "phone";
      authError = "";
      triggerRender();
    });
  ...
  const toEmailSignInBtn = document.getElementById("toEmailSignInBtn");
  if (toEmailSignInBtn) {
    toEmailSignInBtn.addEventListener("click", () => {
      authMode = "signin";
      authError = "";
      triggerRender();
    });
  ...
  // Phone OTP Form Submission (Priority 1 for mobile embroidery marketplace)
  const phoneOtpForm = document.getElementById("phoneOtpLoginForm");
  if (phoneOtpForm) {
    phoneOtpForm.addEventListener("submit", async (e) => {
      e.preventDefault();
      const formData = new FormData(phoneOtpForm);

      if (!otpSent) {
        const phone = (formData.get("phone") || enteredPhone || "").trim();
        ...
        if (!otp || otp.length < 4) {
          authError = "Please enter the verification code received (e.g. 123456).";
          triggerRender();
          return;
        }
        isSubmitting = true;
        ...
        try {
          const success = await loginWithPhoneOtp(enteredPhone, otp);
          isSubmitting = false;
          if (success) {
          ...
          isSubmitting = false;
          authError = err.message || "Failed to verify OTP code.";
          triggerRender();
        }
      }
    });
