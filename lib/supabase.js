...
/**
 * Fetch product by ID, slug, code, or Sanity document ID with authoritative database price
 */
export async function getProductById(productId) {
  if (!productId) return null;
  const selectFields = "id,code,title,price,slug,width,height,total_stitch_count,thread_colors,image,description,design_file";

  // 1. If valid UUID, query by id
  if (UUID_REGEX.test(productId)) {
    try {
      const rows = await supabaseFetch(`/rest/v1/products?id=eq.${encodeURIComponent(productId)}&select=${selectFields}`);
      if (Array.isArray(rows) && rows.length > 0) return rows[0];
    } catch (e) {
      console.warn("Product query by UUID failed:", e.message);
    }
  }

  // 2. Query by slug
  try {
    const rows = await supabaseFetch(`/rest/v1/products?slug=eq.${encodeURIComponent(productId)}&select=${selectFields}`);
    if (Array.isArray(rows) && rows.length > 0) return rows[0];
  } catch (e) {}

  // 3. Query by code
  try {
    const rows = await supabaseFetch(`/rest/v1/products?code=eq.${encodeURIComponent(productId)}&select=${selectFields}`);
    if (Array.isArray(rows) && rows.length > 0) return rows[0];
  } catch (e) {}

  // 4. If productId is a Sanity ID (e.g. prod-1rs-live-test-design), query Sanity to resolve its slug/code
  try {
    const sanityUrl = `https://6l7i886u.api.sanity.io/v2023-08-01/data/query/production?query=${encodeURIComponent(`*[_id == "${productId}"][0]{ slug, code, title, price }`)}`;
    const sRes = await fetch(sanityUrl);
    if (sRes.ok) {
      const sData = await sRes.json();
      const sSlug = sData.result?.slug?.current || sData.result?.slug;
      const sCode = sData.result?.code;

      if (sSlug) {
        const rows = await supabaseFetch(`/rest/v1/products?slug=eq.${encodeURIComponent(sSlug)}&select=${selectFields}`);
        if (Array.isArray(rows) && rows.length > 0) return rows[0];
      }
      if (sCode) {
        const rows = await supabaseFetch(`/rest/v1/products?code=eq.${encodeURIComponent(sCode)}&select=${selectFields}`);
        if (Array.isArray(rows) && rows.length > 0) return rows[0];
      }
    }
  } catch (sanityErr) {
    console.warn("Sanity product resolution fallback error:", sanityErr.message);
  }
...
export async function getPurchaseById(purchaseId) {
  ...
  if (!UUID_REGEX.test(purchaseId)) return null;
...
export async function checkActiveEntitlement({ productId, userId = null, guestTokenHash = null }) {
  ...
  if (!UUID_REGEX.test(productId)) return false;
